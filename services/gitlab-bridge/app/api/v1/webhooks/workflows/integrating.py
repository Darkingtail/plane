"""
Integration workflow (→ Integrating state)

Handles issue state change to Integrating.
Executes feature → dev merge.

Key difference from jira-scripts:
- No Jpom build trigger (Plane version doesn't use Jpom)
...
async def handle_integrating(issue_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Integration workflow: feature → dev merge
    ...
    logger.info(f"[Integrating] Starting workflow for issue {issue_identifier}")

    # 1. Execute merge workflow
    results = await merge_feature_to_branch(
        issue_identifier=issue_identifier,
        issue_data=issue_data,
        target_branch_type="dev",
        workflow_name="Integrating",
    )

    # 2. Handle no branches found
    if not results:
        logger.warning(f"[Integrating] Issue {issue_identifier} has no feature branches")
        project_id = settings.plane_project_id
        await plane_service.add_comment(
            project_id, issue_id,
            "<p>No feature branches found. Cannot auto-merge to dev.</p>"
        )
    ...
    for r in conflict_results:
        await notify_conflict(
            ...
            target_branch=r.get("target_branch", "dev"),
            mr_url=r.get("mr_url", ""),
    ...
    if success_count > 0 or skip_count > 0:
        actual_target = results[0].get("target_branch", "dev") if results else "dev"
        await add_merge_comment(issue_id, actual_target, results)

    logger.info(f"[Integrating] Issue {issue_identifier} workflow completed")
